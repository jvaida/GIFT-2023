...
* this work.
*/
package mil.arl.gift.gateway.interop.ride;
...
/**
 * This is the RIDE interop interface responsible for communicating with RIDE (Rapid Integration & Development Environment)
 * 
 * @author mcambata
 */
public class RIDEPluginInterface extends AbstractInteropInterface {
    
    /** Instance of the logger */
    private static Logger logger = LoggerFactory.getLogger(RIDEPluginInterface.class);
    
    /** The number of bytes used in RIDE's "message start" signature */
    public static final int MESSAGE_START_BYTE_COUNT = 4;
    
    /** 
     * The characters used in RIDE's "message start" signature 
     * The value is defined by RIDE, but set here so it can be searched for and read. */
    public static final Byte[] MESSAGE_START_BYTE_SIGNATURE = new Byte[]{77, 65, 68, 65};
    
    /** The number of bytes used to determine the event type of an incoming RIDE proto message. */
    public static final int EVENT_TYPE_BYTE_COUNT = 2;
    
    /** The number of bytes used to determine the size of an incoming RIDE proto message. */
    public static final int MESSAGE_SIZE_VALUE_BYTE_COUNT = 4;
    
    /** The maximum size that an incoming RIDE message should have. */
    public static final int MAX_RIDE_MESSAGE_SIZE = 1024;
    
    /** default seconds the feedback will stay visible in the RIDE UI */
    private static final float DEFAULT_FEEDBACK_DURATION = (float) 2.0;
    
    /** incoming RIDE health value upper threshold used to set GIFT damage value to {@link #DamageEnum.SLIGHT_DAMAGE} */
    private static final float SLIGHT_DAMAGE_THRESHOLD = (float) 0.75;
    
    /** incoming RIDE health value upper threshold used to set GIFT damage value to {@link #DamageEnum.MODERATE_DAMAGE} */
    private static final float MODERATE_DAMAGE_THRESHOLD = (float) 0.5;
    
    /** incoming RIDE health value upper threshold used to set GIFT damage value to {@link #DamageEnum.DESTROYED_DAMAGE} */
     ...
     * NOTE: This is also the max size of a proto message from RIDE that can be processed.
     */
    private static final int BYTE_INPUT_QUEUE_SIZE = 4096;
    
    /** Default hour of dusk used for sending time of day to RIDE */
    private static final int DEFAULT_DUSK = 18;
    /** Default hour of dawn used for sending time of day to RIDE */
    private static final int DEFAULT_DAWN = 8;
    /** Default hour of midday used for sending time of day to RIDE */
    private static final int DEFAULT_MIDDAY = 12;
    /** Default hour of midnight used for sending time of day to RIDE */
    ...
    /** Used for connecting to RIDE with GRPC */
    private ManagedChannel grpcChannel;
    
    /** 
     * Async stub for GRPC communication regarding RIDE entities.
     * For example: can be used to get an entity type and force ID from RIDE entities, or can set the location of a RIDE entity.
     **/
    private EntityServiceGrpc.EntityServiceStub entityServiceStub;
    
    /**
     * Async stub for GRPC communication regarding RIDE scenario.
     * For example: can be used to send feedback messages to RIDE application from GIFT.
     */
    private ScenarioServiceGrpc.ScenarioServiceStub scenarioServiceStub;
    
    /**
     * Async stub for GRPC communication regarding equipment in RIDE.
     * For example: can be used to cause a weapon malfunction or to repair the functionality of a weapon in RIDE.
     */
    private EquipmentServiceGrpc.EquipmentServiceStub equipmentServiceStub;
    
    /** The byte input from the RIDE application */
     ...
     * an external training applications (e.g. RIDE). 
     */
    ...
    static{
        ...
        REQ_TRAINING_APPS.add(TrainingApplicationEnum.RIDE); 
    }
    
    /** the socket connection */
    private SocketHandler socketHandler = null;
    
    /** configuration parameters for the connection to the GIFT RIDE plugin */
    private generated.gateway.RIDE ridePluginConfig = null;    
     ...
     */
    public RIDEPluginInterface(String name) {
        super(name, true);
    ...
    public boolean configure(Serializable config) throws ConfigurationException {
        ...
        if (config instanceof generated.gateway.RIDE) {
                        
            this.ridePluginConfig = (generated.gateway.RIDE) config;
            createSocketHandler();
            ...
            throw new ConfigurationException("RIDE Plugin interface can't configure.",
                    "The RIDE Plugin interface only uses the interop config type of "+generated.gateway.RIDE.class+" and doesn't support using the interop config instance of " + config,
                    null);
        }
    ...
    /**
     * Determines if this session has been given a domain session ID value corresponding to a currently running RIDE user session.
     * 
     * @return whether or not this session has been given a domain session ID value corresponding to a currently running RIDE user session..
     */
    ...
    private void createSocketHandler(){
        ...
        if(ridePluginConfig != null){
            ...
            if(socketHandler == null || !socketHandler.isConnected()){
                ...
                socketHandler = new SocketHandler(ridePluginConfig.getNetworkAddress(), ridePluginConfig.getNetworkPort(), socketListener, MAX_RIDE_MESSAGE_SIZE);
                
                if(logger.isInfoEnabled()){
                    logger.info("RIDE plugin interface socket handler created.");
                }
            }
        }
    }
    ...
    /**
     * Connects the channel used for GRPC communication with RIDE
     */
    private void connectGrpcChannel() {
        
        if (ridePluginConfig != null) {
            
            String target = ridePluginConfig.getNetworkAddress().toString() + ":" + ridePluginConfig.getGrpcNetworkPort();
    ...
    /**
     * Shuts down the channel used for GRPC connection with RIDE
     */
    ...
    /**
     * Called when byte data is sent to the RIDE plugin interface. Adds the data to a queue so it can later be read and processed.
     * 
     * @param byteInput Incoming data from RIDE, starting in the form of a byte array. This will be converted to a readable format.
     */
    protected synchronized void onSocketReceivedData(byte[] byteInput) {
        try {
            ...
            while (byteInputQueue.size() > 0 && !isByteQueueLessThanMessageSize()) {
                parseProtoMessageFromByteQueue();
            }
            
            int sizeValue = getRIDEMessageSize();
            
            if (sizeValue > MAX_RIDE_MESSAGE_SIZE) {
                logger.error("RIDE protobuf message has an invalid size of " + sizeValue + ". This likely indicates an error in data transfer. Message will be skipped.");
            ...
            logger.error("Error processing data received from RIDE", e);
        }
    }
    
    /**
     * Finds the beginning of the next incoming message from RIDE
     * 
    ...
    private synchronized int getIndexOfNextMessageStartInByteQueue() {
        try {
            ...
            logger.error("An exception occurred while searching for the start of a message from RIDE", e);
        }
    ...
    private synchronized void parseProtoMessageFromByteQueue() {
        try {
            ...
            if (sizeValue > MAX_RIDE_MESSAGE_SIZE) {
                throw new Exception("RIDE protobuf message has an invalid size of " + sizeValue + ". This likely indicates an error in data transfer. Message will be skipped.");
            }
            ...
            try {
                switch (stateEventType)
                {
                    ...
                    case EntityFireWeapon:
                        ...
                        break;
                    default:
                        logger.info("received unhandled RIDE state event type of "+stateEventType+" of size "+sizeValue);
                        break;
                }
            } catch (Exception e) {
                logger.error("Error parsing a RIDE protobuf message of type "+stateEventType+".", e);
            }
                   
        } catch (Exception e) {
            logger.error("Error parsing a RIDE protobuf message", e);
        }
    }
    
    /**
     * Removes trailing zeros at the end of a byte array.
     * This is performed because, when reading incoming bytes from RIDE, there may be 0-bytes after some messages.
    ...
    private synchronized boolean isByteQueueLessThanMessageSize() {
        ...
        int messageSize = getRIDEMessageSize();
    ...
    /**
     * Get the size of the current RIDE message's protobuf bytes. 
     * Requires that the byte input queue start at the beginning of a RIDE message to return correct information.
     * If this is not the case, then returns -1.
     * 
     * @return the size of the current RIDE message's protobuf bytes. Or -1 if the input queue is not at the beginning of a RIDE message.
     */
    private synchronized int getRIDEMessageSize() {
        try {
            ...
            logger.error("Error getting size of an incoming RIDE message", e);
    ...
    /**
     * Requests an entity type and force ID from RIDE via GRPC.
     * If a response is received, that data is sent to the world state manager.
     * If a user session ID has not been defined for the RIDEPluginInterface yet, this does nothing.
     * 
     * The response is received and then processed asynchronously. 
     * This does not prevent the current thread from continuing
     * while it waits for a response from RIDE.
     * 
     * @param simId The int simId used to identify the entity whose data is being requested from RIDE.
     */
            ...
            try {
                ...
                entityServiceStub.getDisType(request, new StreamObserver<EntityFunctions.GetDisTypeResponse>() {
                    ...
                    public void onError(Throwable e) {
                        logger.error("Error getting DIS type from RIDE for entity ID " + simId + ": " + e.toString(), e);
                    ...
                    public void onCompleted() {
                        if(logger.isInfoEnabled()){
                            logger.info("Completed getting DIS type from RIDE.");
                        }
                    }
                });
                                      
            } catch (Throwable e) {
                logger.error("Failed to get a DIS type from RIDE for entity ID " + simId, e);
    ...
    /**
     * Creates an EntityState based on data gathered from a RIDE protobuf message.
     * 
     * RIDE does not sent entity updates with the same data as the EntityState message used by GIFT, so
     ...
     * this method will asynchronously request that data from RIDE, and send the EntityState message
     * with an unknown EntityType and force ID. Once the data is received, the WorldStateManager will
     * associate it with the entity's simulation ID from RIDE, and subsequent EntityState messages
     ...
     */
    private EntityState getEntityStateFromRIDEEntity(EntityOuterClass.EntityTransform transform, EntityOuterClass.Health health) {
...
//        Timestamp time = msg.getWhen();
    ...
    /**
     * Sends a feedback message to RIDE to be displayed in the application
     * 
     * The response is received and then processed asynchronously. 
     * This does not prevent the current thread from continuing
     * while it waits for a response from RIDE.        
    ...
    private void sendDisplayFeedBackMessage(String message, float duration) {
        ...
        if (message == null) {
            logger.error("Tried to send a Display Feedback message to RIDE with a null message. Message must be a non-null string value.");
            return;
        }
        
        if (duration <= 0.0) {
            logger.error("Tried to send a Display Feedback message to RIDE with a duration of " + duration + ". Duration must be a float greater than 0.");
            return;
        }
        ...
        try {
            ...
            scenarioServiceStub.provideFeedback(request, new StreamObserver<ScenarioFunctions.ProvideFeedbackResponse>() {
                ...
                public void onNext(ProvideFeedbackResponse response) {
                    if(logger.isInfoEnabled()){
                        logger.info("Feedback received by RIDE: " + message);
                    }
                ...
                public void onError(Throwable t) {
                    logger.error("Error sending feedback to RIDE", t);
            ...
            logger.error("Failed to send a DisplayFeedBackMessage to RIDE", e);
        }
    }
    
    /**
     * Sends a message to RIDE to disable the weapon of the specified entity.
     * 
     * The response is received and then processed asynchronously. 
     * This does not prevent the current thread from continuing
     * while it waits for a response from RIDE.
    ...
    private void sendWeaponMalfunction(int simId) {
        try {
            ...
            equipmentServiceStub.getPrimaryWeapon(weaponRequest, new StreamObserver<EquipmentFunctions.GetPrimaryWeaponResponse>() {
                ...
                public void onNext(GetPrimaryWeaponResponse weaponResponse) {
                    ...
                    equipmentServiceStub.malfunction(malfunctionRequest, new StreamObserver<EquipmentFunctions.MalfunctionResponse>() {
                        ...
                        public void onNext(MalfunctionResponse malfunctionResponse) {
                            if(logger.isInfoEnabled()){
                                logger.info("Malfunction inject received by RIDE for entity ID: " + simId);
                            }
                        ...
                        @Override
                        public void onError(Throwable t) {
                            logger.error("Error sending malfunction to RIDE", t);
                ...
                @Override
                public void onError(Throwable t) {
                    logger.error("Error getting a primary weapon from RIDE", t);
            ...
            logger.error("Failed to send a Weapon Malfunction inject to RIDE", e);
        }
        
    }
    
    /**
     * Sends a message to RIDE to repair the weapon of the specified entity.
     * 
    ...
    private void sendEntityRepairObjectMessage(int simId) {        
        try {
                ...
                public void onNext(GetPrimaryWeaponResponse weaponResponse) {
                    ...
                    equipmentServiceStub.repair(repairRequest, new StreamObserver<EquipmentFunctions.RepairResponse>() {
                        ...
                        public void onNext(EquipmentFunctions.RepairResponse repairResponse) {
                            if(logger.isInfoEnabled()){
                                logger.info("Repair inject received by RIDE for entity ID: " + simId);
                            }
                        ...
                        @Override
                        public void onError(Throwable t) {
                            logger.error("Error sending repair to RIDE", t);
            ...
            logger.error("Failed to send a Repair inject to RIDE", e);
        }
    }
    
    /**
     * Sends a target time, as a protobuf Timestamp, rounded down to the nearest second, to RIDE.
     * RIDE should set the time in the scenario to this time.
     * 
        ...
        try {
            ...
            scenarioServiceStub.setTimeOfDay(timeOfDayRequest, new StreamObserver<ScenarioFunctions.SetTimeOfDayResponse>() {
                ...
                public void onNext(SetTimeOfDayResponse arg0) {
                    if(logger.isInfoEnabled()){
                        logger.info("'Set Time of Day' inject received by RIDE for time stamp: " + timeOfDayStamp);
                    }
                }
                
                @Override
                public void onError(Throwable t) {
                    logger.error("Error sending 'Set Time of Day' inject to RIDE: " + timeOfDayStamp, t);
                ...
                public void onCompleted() {
                    if(logger.isInfoEnabled()){
                        logger.info("'Set Time of Day' inject to RIDE was completed for time stamp: " + timeOfDayStamp);
                    }
                }
            });
             
        } catch (Exception e) {
            logger.error("Failed to send a 'Set Time of Day' inject to RIDE", e);
        }
    }
     ...
     * then produces a Timestamp object that can be sent to RIDE.
     * 
     ...
     * @return A protobuf TimeOfDay object which can be sent to RIDE
    ...
    private Timestamp convertTimeOfDayToTimeStamp(generated.dkf.EnvironmentAdaptation.TimeOfDay targetTimeOfDay, long currentTimeMilliseconds) throws IllegalArgumentException {
        ...
        try {
            ...
            Date RIDEDate = new Date(currentTimeMilliseconds);
            Calendar RIDECalendar = GregorianCalendar.getInstance();
            RIDECalendar.setTime(RIDEDate);
            int currentHourOfDay = RIDECalendar.get(Calendar.HOUR_OF_DAY);

            //calculate the amount of time to skip forward in RIDE
            long skipTime = 0;
            ...
            logger.error("Caught an exception while changing the time of day. Defaulting to current simulated RIDE time. ", e);
            
            long currentTimeSeconds = currentTimeMilliseconds / MILLISECONDS_IN_SECOND;
            
            Timestamp currentRIDETimestamp = Timestamp.newBuilder().setSeconds(currentTimeSeconds).build();
            
            return currentRIDETimestamp;
        }
    }
...
//        EntityOuterClass.SimulationStateType state = msg.getState();
     ...
     * for related data, due to the fact that RIDE produces more limited information than is used by the
     * Detonation and WeaponFire messages. As incoming data from RIDE is improved, this can be adjusted to record more precise data.
     * 
    ...
    private void handleEntityFireWeaponProtoMessage(EntityEvents.EntityFireWeaponMessage msg) {
        ...
        /* 
         * Default/unknown values are used in cases where data from RIDE's EntityFireWeaponMessage does not
        ...
        EventIdentifier eventId = new EventIdentifier(new SimulationAddress(0, 0), 0);
        EntityType entityType = new EntityType(0, 0, 0, 0, 0, 0, 0);
    ...
    /**
     * Determines the entity type of a RIDE protobuf message from a byte array.
     * 
     * @param stateByteArray the byte array
     * @return the EventType of the RIDE protobuf message.
     */
    ...
    /**
     * Determines the size (in bytes) of a RIDE protobuf message from a byte array.
     * 
    ...
    public void setEnabled(boolean value) throws ConfigurationException{
        ...
        if(isEnabled() && !value){
            ...
            if(logger.isInfoEnabled()){
                logger.info("Disabling RIDE plugin interface");
            }
            ...
            try {
                ...
                if(logger.isDebugEnabled()){
                    logger.debug("Exception occurred while disabling RIDE plugin. This should not cause an error due to when it occurs.", e);
                }
            }
            ...
            if(logger.isInfoEnabled()){
                logger.info("Enabling RIDE plugin interface");
            }
            ...
            try {
                ...
                throw new ConfigurationException("Failed to connect the RIDE Plugin interface socket connection.", 
    ...
    public boolean handleGIFTMessage(Message message, StringBuilder errorMsg) throws ConfigurationException {
        ...
        if (message.getMessageType() == MessageTypeEnum.ENVIRONMENT_CONTROL) {
            ...
            logger.error("Received unhandled GIFT message to send over to the RIDE plugin, " + message);            
            errorMsg.append("RIDE plugin can't handle message of type ").append(message.getMessageType());
        }
    ...
    /**
     * Apply the scenario adaptation using RIDE scripting command.
     * 
    ...
    private void applyEnvironmentControl(EnvironmentControl control, StringBuilder errorMsg) {
        ...
        if(type instanceof generated.dkf.EnvironmentAdaptation.Script){
            ...
            if(logger.isDebugEnabled()){
                logger.debug("sending command to RIDE plugin:\n"+script.getValue());
            }
            ...
            try {
                ...
                /* 
                 * Get the current time in milliseconds. In the future, if RIDE offers a request for current time, we can modify
                 * this to start from RIDE's current time instead.
                 */
                ...
                logger.error("Error sending a Time of Day to RIDE.", e);
            }
        }
    ...
    /**
     * Parse a script command to RIDE GIFT Plugin, convert it to a protobuf message,
     * and send the resulting method to the external RIDE application.
     * <br/>
     ...
     * 
     * @param value The RIDE command to parse in the RIDE plugin and then send to the external RIDE application. Can't be null.
        ...
        if (scriptCommandSignature.length > 2) {
            ...
            if (commandName.equalsIgnoreCase(DISABLE_WEAPON_CONTROL_SCRIPT_NAME)) {
                if (scriptArgsSplit.length >= 1) {
                    ...
                    logger.error("Command to RIDE does not have enough arguments for disableWeaponControl.");
                    errorMsg.append("Command to RIDE does not have enough arguments for disableWeaponControl. " + scriptArgsSplit.length + " arguments defined. 5 are required.");
                }
            }
            else {
                logger.error("script command to RIDE of \"" + commandName + "\" was not recognized as a valid command.");
                errorMsg.append("script command to RIDE of \"" + commandName + "\" was not recognized as a valid command.");
            }
        } else {
            logger.error("Command to RIDE improperly formatted and could not be sent.");
            errorMsg.append("Command to RIDE improperly formatted and could not be sent.");

        }
    }
    
    /**
     * Sends a command to RIDE to either cause a weapon to malfunction or to repair the weapon of a given entity.
     * 
